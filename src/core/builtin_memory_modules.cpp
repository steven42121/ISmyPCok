...
public:
    ...
    ModuleResult run() override
    {
        ...
        const double wall = RunParallel(threads, [&](unsigned t)
        {
            const std::size_t chunk = size / threads;
            const std::size_t offset = static_cast<std::size_t>(t) * chunk;
            for (int i = 0; i < loops; ++i)
                std::memcpy(dst.data() + offset, src.data() + offset, chunk);
        });

        const double gib = (static_cast<double>(size) * loops) / (1024.0 * 1024.0 * 1024.0);
        const double gibps = gib / wall;

        result.metrics["gibps"] = gibps;
        result.metrics["elapsed_s"] = wall;
        result.metrics["threads"] = static_cast<double>(threads);
        result.score = ClampScore(gibps * 3.0);
...
public:
    ...
    ModuleResult run() override
    {
        ...
        result.score = ClampScore(100.0 - (ns_per_access - 70.0) * 2.0);
